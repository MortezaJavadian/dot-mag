...
export function Header() {
  ...
  const handleMobileMenuOutsidePointerDown = (
  ...
  const handleMobileMenuPanelPointerDown = (
    event: PointerEvent<HTMLDivElement>,
  ) => {
    event.stopPropagation();
  };
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-x-0 bottom-0 top-16 z-[120] transition-opacity duration-300 md:hidden ${
          isMenuOpen
            ? "opacity-100 pointer-events-auto"
            : "opacity-0 pointer-events-none"
        }`}
        onPointerDown={handleMobileMenuOutsidePointerDown}
      >
        <div
          aria-hidden="true"
        ...
        <nav className="container relative z-10 py-6">
          <div
            className="rounded-[1.35rem] border p-5 shadow-[0_28px_64px_rgba(0,0,0,0.44)]"
            style={{
